...
def main():
    ...
    for build_model in (h.build_lstm_model, h.build_gru_model):
...
class Helper:
    ...
    def build_lstm_model(self, embedding_matrix, num_aux_targets):
        ...
        hidden = add([hidden, Dense(4 * LSTM_UNITS, activation='relu')(hidden)])
        hidden = add([hidden, Dense(4 * LSTM_UNITS, activation='relu')(hidden)])
    ...
    def build_gru_model(self, embedding_matrix, num_aux_targets):
        words = Input(shape=(self.config.max_len,))
        x = Embedding(*embedding_matrix.shape, weights=[embedding_matrix], trainable=False)(words)
        x = SpatialDropout1D(0.3)(x)
        ...
        hidden = add([hidden, Dense(4 * GRU_UNITS, activation='relu')(hidden)])
        hidden = add([hidden, Dense(4 * GRU_UNITS, activation='relu')(hidden)])
        result = Dense(1, activation='sigmoid')(hidden)
        aux_result = Dense(num_aux_targets, activation='sigmoid')(hidden)
